...
class Trainer:
    def __init__(self, profiles):
        ...
        if args.pretrain_emb:
            ...
            args.enc_emb_dim = embeddings.shape[1]
            args.dec_emb_dim = embeddings.shape[1]
    ...
    def parse_args(self):
        ...
        parser.add_argument('--pretrain_emb', action='store_true', required=False, help='when it is true, emb_dim will be ignored and set to pretrain_emb dim')
    ...
    def build_model(self, embeddings):
        ...
        attention = utils.Attention(args.enc_hid_dim, args.enc_emb_dim*2, 
                args.attn_dim)
        profile_detector = modules.ProfileDetector(
                input_dim, args.enc_emb_dim, args.enc_hid_dim,
                len(self.profiles_features), attention, args.enc_dropout,
                args.emb_freeze, pad_idx, embeddings
                )
        ...
        attention = utils.Attention(args.enc_hid_dim, args.dec_hid_dim, 
                args.attn_dim)
    ...
    def train(self, data_iter=None, early_stage=False):
        ...
        for _, (X, y, X_lens, y_lens, profile_key) in enumerate(data_iter):
            ...
            X = X.to(self.device)
            y = y.to(self.device)
            profile_key = profile_key.float().to(self.device)
            profile_y = profile_key[:, 0]
            profile_j = profile_key[:, 1]
            ...
            out_loss = torch.tensor(0.).to(self.device)
            profile_exists_loss = torch.tensor(0.).to(self.device)
            profile_detector_loss = torch.tensor(0.).to(self.device)
            ...
            if exists_has_profile:
                ...
                bi_out = torch.where(b_out == 0, f_out, b_out)
                bi_out[v_pos, torch.arange(bi_out.shape[1]), y_pos] = 1
