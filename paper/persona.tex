...
字分割的词汇大小9489，嵌入维度200，词分割的个性词汇大小10004，个性嵌入维度与模型隐藏维相同512，

基线AR模型的参数与AR+一致。

\subsection[Datasets]{数据集} 
PersonalDialog数据集原始数据包含部分重复数据，去重后是5,195,149个会话，训练、验证和测试数据集是分别在数据去重后随机无置回取出的10万会话子集。我们用验证集调整超参数，在测试集上进行评测。发言平均长度为15个字。
