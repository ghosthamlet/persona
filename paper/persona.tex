...
字分割的词汇大小为9489，嵌入维度为200，词分割的个性词汇大小为10004，个性嵌入维度与模型隐藏维度相同为512，Transformer层数为6，Attention头为8，全连接维度为2048。通过lr finder\citeext{Smith2015}(类库\footnote{\url{https://github.com/davidtvs/pytorch-lr-finder}})发现学习率可为0.2e-2，权重衰退为0.05，clip grad为1，语言模型的$L_{LM}~\lambda_1$为0.5，学习率调度器为ReduceLROnPlateau，其参数分别为：mode min, factor 0.5, min\_lr 1.5e-4, patience 60，MemN2N的参数分别为：hops 3，layer\_share adjacent。
